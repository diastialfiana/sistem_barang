<x-app-layout>
    <div class="max-w-6xl mx-auto" x-data="requestForm()">
        ...
        <div class="mb-8">
            ...
            <p class="mt-2 text-slate-500 font-medium">
                Perbarui detail request barang #{{ $request->code }}.
            </p>
        </div>
        ...
        <div class="bg-white rounded-3xl shadow-[0_20px_50px_rgba(0,0,0,0.04)] border border-slate-100 overflow-hidden relative">
            ...
            <form action="{{ route('requests.update', $request->id) }}" method="POST" class="p-8 md:p-10 relative z-10">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-8 mb-10 pb-8 border-b border-slate-50">
                    <div class="md:col-span-1">
                        ...
                        <input type="date" name="request_date" value="{{ old('request_date', $request->request_date) }}" 
                     ...
                     <div class="md:col-span-2">
                        ...
                        <input type="text" readonly value="{{ optional($request->branch)->name ?? 'Dept. Operasional' }}" 
    ...
    <!-- Alpine Logic -->
    @php
        $mappedRows = $request->items->map(function($item) {
            return [
                'id' => $item->id,
                'item_name' => $item->item_name,
                'quantity' => $item->quantity,
                'stockDisplay' => optional($item->item)->stock ?? '-',
                'maxStock' => optional($item->item)->stock ?? 999999,
                'unit' => optional($item->item)->unit ?? '',
    ...
    <script>
        function requestForm() {
            return {
                ...
                rows: @json($mappedRows),
                
                init() {
                    this.rows.forEach((row, index) => {
                         this.updateStock(index, true);
                    });
                },
                ...
                updateStock(index, initial = false) {
