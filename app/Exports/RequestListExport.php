...
class RequestListExport implements FromCollection, WithHeadings, WithEvents, WithStyles
{
    protected $request;
    protected $branches;
    ...
    protected function getBranches()
    {
        ...
        if ($this->request->has('location_type') && in_array($this->request->location_type, ['dalam_kota', 'luar_kota'])) {
        ...
        if ($this->request->has('month') && $this->request->month) {
            $date = \Carbon\Carbon::createFromFormat('Y-m', $this->request->month);
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function(AfterSheet $event) {
                ...
                $overallStatus = 'APPROVED'; // Forced as per request
                ...
                // Determining "Requester" for the Header
                $requesterUser = null;
                ...
                if ($this->request->has('location_type') && in_array($this->request->location_type, ['dalam_kota', 'luar_kota'])) {
                     $query->whereHas('branch', function($q) { $q->where('location_type', $this->request->location_type); });
                }
                ...
                if (!$requesterUser) {
                     ...
                     if ($this->request->has('location_type')) {
                         $branchIds->where('location_type', $this->request->location_type);
                     }
                ...
                if ($this->request->has('month') && $this->request->month) {
                    $dateObj = \Carbon\Carbon::createFromFormat('Y-m', $this->request->month);
                ...
                // Requested Mapping:
                // Column B: Mengetahui (SPV)
                // Column C: Mengetahui (KA)
                // Column D: Disetujui (GA)
                $colSPV = 'B';
                $colKA = 'C';
                $colGA = 'D';
                ...
                $gaUser = \App\Models\User::role('super_admin')->first();
                 ...
                 // Determine branch from query to correct the SPV signature if possible
